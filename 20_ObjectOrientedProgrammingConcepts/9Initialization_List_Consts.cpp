...
public:
    ...
    const int msp;      //it is fixed and read only ; operators not work on this like --> msp++;
    ...
    //this fun is constant fun bcz it does not update the datamember
    float get_discount(const float x){
        return price*(1-x);
    }

    //this doest have const fun bcz it update price but have constant data member
